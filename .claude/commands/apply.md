...
- `docs/ARCHITECTURE.md`: how it is built and where a new file goes, from the real traces. Unwired code named as unwired, once, with the open question it raises.
  **If this file is already theirs**, you may not write it, so write `docs/TECH_STACK.md` instead of deleting it and put the tool-versus-alternative reasons there. Measured on 2026-09-21: a repository with its own `ARCHITECTURE.md` scored 5 of 5 before this run and 4 of 5 after, because the only two files that can carry "why this tool and not the obvious one" were one skipped and one deleted, and the question stopped being answerable. A lean run must never leave a question less answerable than it found it.
- `docs/CONVENTIONS.md`: the patterns the code actually keeps. When their repo has a reviewer's own instruction sheet (`.github/copilot-instructions.md`, a `CODEOWNERS` review guide), say which of its sections is the list a reviewer flags, and rank this file the way that file ranks it.
...
Then delete the templates this run skipped, `docs/HANDOFFS.md`, `docs/DESIGN.md`, `docs/context/CONTINUE_PROMPT.md` and `docs/TECH_STACK.md` unless the rule above kept it, and drop their pointer lines from the root instruction file's context block. `docs/README.md` already marks which files a lean run skips, so leave it alone. Delete only while they still carry `{{placeholders}}`, which proves they are the kit's copies, not the human's. `/apply-airtight` writes all nine.
